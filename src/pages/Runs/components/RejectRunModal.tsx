...
import type { RejectRunModalProps } from "../types"

export const RejectRunModal: React.FC<RejectRunModalProps> = ({
  isOpen,
  run,
  reason,
  onReasonChange,
      ...
      <DialogContent className="bg-slate-900 border border-slate-800 text-slate-100 max-w-md">
        <DialogHeader className="space-y-2">
          <div className="flex items-center gap-2 text-rose-400 font-semibold text-base">
            <X className="h-5 w-5" />
            <DialogTitle className="text-white text-base">Tolak Run</DialogTitle>
          </div>
          <DialogDescription className="text-slate-400 text-xs leading-relaxed">
            Penolakan run <strong className="text-white font-mono">{run?.task?.id}</strong> akan membersihkan worktree dan mengarsipkan catatan audit.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={onConfirm} className="space-y-4">
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              placeholder="Masukkan alasan penolakan run..."
          ...
          <DialogFooter className="border-slate-800 bg-slate-900/50 -mx-4 -mb-4 p-4 rounded-b-xl">
            ...
            >
              ...
              <span>{actionLoading ? "Menolak..." : "Tolak Run"}</span>
            </button>
          </DialogFooter>
        </form>
      </DialogContent>
