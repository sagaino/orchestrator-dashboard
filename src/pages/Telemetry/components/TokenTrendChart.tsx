...
const CustomChartTooltip: React.FC<CustomTooltipProps> = ({ active, payload, label }) => {
  ...
  return (
    <div className="rounded-lg border border-slate-800 bg-slate-950/95 p-3 text-xs shadow-xl backdrop-blur-md space-y-2 min-w-[200px] max-w-[280px] z-50">
      <div className="border-b border-slate-800 pb-1.5 flex items-center justify-between gap-2">
        ...
        {point?.stage && (
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-slate-800 text-slate-300 font-mono font-medium shrink-0">
            {point.stage}
          </span>
        )}
      </div>
      ...
      <div className="border-t border-slate-800/80 pt-1.5 flex justify-between text-[10px] text-slate-400 font-mono">
        <span className="truncate max-w-[120px]">Model: {point?.model ? point.model.replace("gemini-", "") : "gemini"}</span>
        <span className="shrink-0">{point?.durationSeconds ? `${point.durationSeconds.toFixed(1)}s` : "-"}</span>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="p-5 sm:p-6 rounded-xl bg-slate-900 border border-slate-800 space-y-4 min-w-0 w-full overflow-hidden h-full flex flex-col justify-between">
      <div className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 min-w-0">
          <div className="space-y-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <h2 className="text-base font-semibold text-white truncate">Token Usage Trends</h2>
          ...
          <div className="flex items-center shrink-0 self-start sm:self-auto bg-slate-950 p-1 rounded-lg border border-slate-800 text-xs">
            <button
              type="button"
        ...
        {/* Mini summary badges */}
        <div className="flex flex-wrap items-center gap-2 pt-1 text-[11px] font-mono min-w-0">
      ...
      {data.length === 0 ? (
        ...
        <div className="h-72 sm:h-80 w-full pt-2 min-w-0 flex-1">
